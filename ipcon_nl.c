...
static int ipcon_grp_reg(struct sk_buff *skb)
{
	int ret = 0;
	struct ipcon_peer_node *ipn = NULL;
	int id = 0;
	...
	ipd_rd_lock(ipcon_db);
	do {
		ipn = ipd_lookup_bycport(ipcon_db, ipconmsg_sndport(skb));
		if (!ipn) {
			ipcon_err("No port %lu found\n.",
				(unsigned long)ipconmsg_sndport(skb));
			ret = -ESRCH;
			break;
		}
...
static int ipcon_grp_unreg(struct sk_buff *skb)
{
	int ret = 0;
	struct ipcon_peer_node *ipn = NULL;
	struct ipcon_group_info *igi = NULL;
	...
	do {
		...
		ipn = ipd_lookup_bycport(ipcon_db,
				ipconmsg_sndport(skb));
		if (!ipn) {
			ret = -ESRCH;
			break;
		}
...
static int ipcon_grp_reslove(struct sk_buff *skb)
{
	int ret = 0;
	struct ipcon_peer_node *ipn = NULL;
	struct ipcon_group_info *igi = NULL;
	...
	if (!ret) {
		do {
			...
			ipconmsg_end(msg, p);
			ret = ipcon_unicast(msg, ipconmsg_sndport(skb));
...
static int ipcon_unicast_msg(struct sk_buff *skb)
{
	int ret = 0;
	char name[IPCON_MAX_NAME_LEN];
	struct ipcon_peer_node *ipn = NULL;
	...
	do {
		void *p = NULL;
		struct ipcon_peer_node *snd_ipn = NULL;
		...
		tport = ipn->port;
		snd_ipn = ipd_lookup_byport(ipcon_db, ipconmsg_sndport(skb));
		if (!snd_ipn) {
			ret = -ESRCH;
			break;
		}
...
static int ipcon_multicast_msg(struct sk_buff *skb)
{
	...
	do {
		...
		ipn = ipd_lookup_byport(ipcon_db, ipconmsg_sndport(skb));
		if (!ipn) {
			ret = -ESRCH;
			break;
		}
		 ...
		 */
		if (sync) {
			ret = ipcon_multicast(msg, ipconmsg_sndport(skb),
			...
			imwd = iw->data;
			imwd->igi = igi;
			imwd->sender_port = ipconmsg_sndport(skb);
			imwd->skb = msg;
...
static int ipcon_peer_reg(struct sk_buff *skb)
{
	int ret = 0;
	struct ipcon_peer_node *ipn = NULL;
	struct ipcon_kevent *ik;
	...
	if (!tb[IPCON_ATTR_PEER_NAME] || !tb[IPCON_ATTR_PORT])
		return -EINVAL;

	nla_strlcpy(name, tb[IPCON_ATTR_PEER_NAME], IPCON_MAX_NAME_LEN);
	com_port = nla_get_u32(tb[IPCON_ATTR_PORT]);
...
static int ipcon_kernel_init(void)
{
	...
	do {
		...
		ipn_kernel = ipn_alloc(0, 0, kpeer_nameid,
...
static int ipcon_ctl_handler(struct sk_buff *skb)
{
	...
	do {
		switch (ipconmsg_cmd(skb)) {
		case IPCON_PEER_REG:
			ret = ipcon_peer_reg(skb);
			break;
		case IPCON_PEER_RESLOVE:
			ret = ipcon_peer_reslove(skb);
			break;
		case IPCON_GRP_REG:
			ret = ipcon_grp_reg(skb);
			break;
		case IPCON_GRP_UNREG:
			ret = ipcon_grp_unreg(skb);
			break;
		case IPCON_GRP_RESLOVE:
			ret = ipcon_grp_reslove(skb);
			break;
		default:
			ret = -EINVAL;
		};
...
static int ipcon_msg_handler(struct sk_buff *skb)
{
	int ret = 0;
	...
	do {
		switch (ipconmsg_cmd(skb)) {
		case IPCON_USR_MSG:
			ret = ipcon_unicast_msg(skb);
			break;
		case IPCON_MULTICAST_MSG:
			ret = ipcon_multicast_msg(skb);
			break;
		default:
			ret = -EINVAL;
		};
